...
#### Commit [58682dab8f1cc59e09fd01df28a35c7dcacc631a](https://github.com/retverd/demo-open-spec-with-cursor/commit/58682dab8f1cc59e09fd01df28a35c7dcacc631a)
...
- exceptions в коде не отлавливаются, возвращаются пользователю, вместо корректного описания ошибки;
...
Попробую починить проблемы в тестах, о которых писал раньше:

```prompt
[Model GPT-5.1 Codex Max] Проверь все py-файлы линтером на ошибки и исправь все проблемы.
```

Этого также явно не хватает в инструкциях.

> Линтерные ошибки в tests/test_cbr_usdrub.py устранены:
>
> - `DummyClient` теперь принимает списки `float | None | Exception`.
> - В `HTTPStatusError` добавлен реальный `Request`, тип `response` приведён для Pyright.
>
> Повторный прогон `read_lints` — без ошибок. Других py-файлов линтер не ругает.

Качество исполнения оставляет желать лучшего, попробую сделать другой моделью.
