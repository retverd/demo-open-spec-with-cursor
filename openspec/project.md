...
- Сохранение собранных данных **локально в файлы** (конкретный формат будет выбран и зафиксирован при реализации).
...
Текущий стек (уже подтверждено в `JOURNAL.md`):

- **OpenSpec CLI**: 0.16.0
- **Python**: 3.14.2 (для запуска OpenSpec)
...
- **Python** (реализация сбора/агрегации/сохранения данных)
- Запуск: **локально**
...
- OpenSpec (CLI и формат спецификаций).
- Cursor Pro (среда работы с агентом).
- GitHub (хостинг репозитория, история изменений).
...
- На текущем этапе предполагаем, что используются **публичные** источники без ключей.
- Если появятся ключи — фиксируем их хранение через переменные окружения и/или секреты CI.
